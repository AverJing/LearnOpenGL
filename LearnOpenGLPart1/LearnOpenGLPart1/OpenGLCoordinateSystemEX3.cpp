...
*
*@author£ºAver Jing
*@description£ºTry to make every 3rd container (including the 1st) rotate over time, 
...
int main()
{
	...
	// ------------------------------------
	Shader ourShader("coordinateSystemCube.vs", "coordinateSystemCube.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 10; ++i) {
			...
			if (i < 3) {
				model = glm::rotate(model, (float)(10.0f * glm::radians(glfwGetTime())), glm::vec3(1.0f, 0.0f, 0.0f));
			}
			else {
				float angle = 20.0f * i;
